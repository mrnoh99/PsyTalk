...
--   · 채팅·일정·자료실 데이터 + 당직표(ward_duty) 전부 삭제(깨끗한 테스트 환경)
...
BEGIN
  ...
  -- 당직표 비우기 (updated_by → profiles FK 때문에 회원 삭제 전에 먼저 비움)
  DELETE FROM public.ward_duty;
...
SELECT count(*) AS ward_duty FROM public.ward_duty;
