...
| (웹) 가입 시 핸드폰 종류·연결 이메일 + 회원 명단 엑셀 다운로드 | ➖ | ✅ 웹 전용 | ➖ | `signup_device.sql`(device_type·device_email). 전체관리자 콘솔에서 CSV 내보내기(가입일·핸드폰 최종변경일 포함, TestFlight/내부테스터용) |
| 내 정보에서 핸드폰 종류·앱 설치 이메일 변경 (전화번호 읽기전용) | ➖ | ✅ 세 플랫폼 | ✅ | `profile_phone_edit.sql`(moim_update_my_profile +device_type·device_email). 전화번호는 변경 불가 |
